...
function exportExecutivePpt(){
  ...
  const dist=[['At Risk',risk,C.amber],['Completed',completed,C.green],['Delayed',delayed,C.red],['On Track',ontrack,C.cyan]],sum=Math.max(1,total),cx=3.18,cy=4.0,R=1.45,angleStart=-90;
  let a=angleStart;dist.forEach(d=>{let ang=360*d[1]/sum;let pts=[];pts.push({x:cx,y:cy});for(let j=0;j<=24;j++){let q=(a+ang*j/24)*Math.PI/180;pts.push({x:cx+R*Math.cos(q),y:cy+R*Math.sin(q)})}sl.addShape(pptx.ShapeType.arc,{x:0,y:0,w:0,h:0,fill:{color:'FFFFFF',transparency:100},line:{color:'FFFFFF',transparency:100}});sl.addShape(pptx.ShapeType.pie,{x:cx-R,y:cy-R,w:2*R,h:2*R,adjustPoint:.25,fill:{color:d[2]},line:{color:d[2]}});a+=ang});
