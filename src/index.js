...
function render(){let m=metrics();document.getElementById('kpis').innerHTML=[['Total',m.total,'blue'],['Delayed / Overdue',m.over,'red'],['At Risk',m.risk,'amber'],['Completed',m.done,'green']].map(x=>'<div class="card"><div class="k">'+x[0]+'</div><div class="v '+x[2]+'">'+x[1]+' <span class="pct">('+pct(x[1],m.total)+')</span></div><div class="mini muted">of '+m.total+' total activities</div></div>').join('');let a=[];if(!m.total)a.push(['critical','No delivery tracker loaded','Import the current project tracker to start PMO control.']);if(m.over)a.push(['critical',m.over+' delayed / overdue commitment(s)','Confirm root cause, accountable PIC, corrective action and recovery ETA.']);if(m.risk)a.push(['',m.risk+' at-risk item(s)','Validate mitigation, dependency and decision date.']);if(m.missing)a.push(['',m.missing+' governance gap(s)','PIC or ETA is missing / TBC.']);if(!a.length)a.push(['good','Portfolio is currently stable','No delayed or at-risk task detected.']);document.getElementById('alerts').innerHTML=a.map(x=>'<div class="alert '+x[0]+'"><b>'+esc(x[1])+'</b><br><span class="mini muted">'+esc(x[2])+'</span></div>').join('');
const dashboardProjects=document.getElementById('dashboardProjects');if(dashboardProjects){dashboardProjects.innerHTML=state.projects.map(p=>{const ts=p.tasks||[],tot=ts.length,del=ts.filter(x=>/delayed|overdue/i.test(x.status)).length,riskN=ts.filter(x=>/risk/i.test(x.status)).length,done=ts.filter(x=>/complete/i.test(x.status)).length,pc=pct(done,tot);return '<div class="project-card '+(p.id===state.active?'active':'')+'" data-dashboard-project="'+esc(p.id)+'"><h4>'+esc(p.name)+'</h4><div class="project-file">'+esc(p.fileName||'No workbook uploaded')+'</div><div class="project-stats"><div class="project-stat"><b class="blue">'+tot+'</b><span>Tasks</span></div><div class="project-stat"><b class="red">'+del+'</b><span>Delayed</span></div><div class="project-stat"><b class="amber">'+riskN+'</b><span>At Risk</span></div><div class="project-stat"><b class="green">'+done+'</b><span>Completed</span></div></div><div class="project-progress"><i style="width:'+pc+'"></i></div><div class="mini muted" style="margin-top:7px">Completion '+pc+'</div></div>'}).join('')||'<div class="muted mini">No projects yet. Upload a project tracker to start.</div>';dashboardProjects.querySelectorAll('[data-dashboard-project]').forEach(e=>e.onclick=()=>{state.active=e.dataset.dashboardProject;save();render()})};let now=project().tasks.filter(x=>/delayed|overdue|risk/i.test(x.status)).slice(0,10);document.getElementById('today').innerHTML=now.length?now.map(x=>'<div class="projectline"><b>'+esc(x.task)+'</b><div class="mini muted">'+esc(x.pic)+' · '+esc(x.eta)+' · '+esc(x.status)+'</div></div>').join(''):'<div class="muted mini">No immediate recovery item detected.</div>';
...
async function analyseUniversalFile(file,projectId){
  ...
  const out=document.getElementById('documentAnalysis');
  if(meta)meta.textContent='Analysing '+file.name+'...';
  if(out)out.innerHTML='<div class="alert">Reading document and building expert analysis...</div>';
  const fd=new FormData();fd.append('file',file);fd.append('project',projectId||project().name);
  try{
    const r=await fetch('/api/document-analyze',{method:'POST',body:fd});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'Document analysis failed');
    if(out)out.innerHTML=renderDocumentAnalysis(d.analysis||d.report||'No analysis returned.');
    if(meta)meta.textContent=file.name+' · '+(d.mode==='ai'?'AI Expert Analysis':'Local Analysis');
    const p=state.projects.find(x=>x.id===projectId)||project();p.documents=p.documents||[];p.documents.push({name:file.name,type:file.type||'unknown',size:file.size,analysedAt:Date.now(),analysis:d.analysis||d.report||''});p.latestDocumentAnalysis=d.analysis||d.report||'';save();render();
    return d;
  }catch(e){if(out)out.innerHTML='<div class="alert critical"><b>Analysis unavailable</b><br><span class="mini muted">'+esc(e.message)+'</span></div>';if(meta)meta.textContent=file.name+' · analysis unavailable';throw e}
}
function escHtml(v){return String(v==null?'':v).replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
function renderRichAdvisor(data){
  const d=data&&typeof data==='object'?data:{reply:String(data||'')};
  const blocks=[];
  if(d.reply)blocks.push('<div class="rich-block"><div class="rich-copy">'+escHtml(d.reply)+'</div></div>');
  (Array.isArray(d.visuals)?d.visuals:[]).forEach(v=>{
    if(!v)return;
    if(v.type==='chart'){
      const labels=Array.isArray(v.labels)?v.labels:[],values=Array.isArray(v.values)?v.values.map(Number):[];
      const max=Math.max(1,...values),w=560,h=Math.max(150,labels.length*42+45),rows=labels.map((l,i)=>{
        const y=30+i*42,b=Math.max(0,values[i]||0),bw=Math.round((b/max)*390);
        return '<text x="0" y="'+(y+13)+'" fill="#bcd0e5" font-size="11">'+escHtml(l).slice(0,34)+'</text><rect x="145" y="'+y+'" width="'+bw+'" height="22" rx="5" fill="#5791ff"/><text x="'+(155+bw)+'" y="'+(y+15)+'" fill="#eef5fc" font-size="11">'+escHtml(b)+'</text>';
      }).join('');
      blocks.push('<div class="rich-block"><div class="rich-title">'+escHtml(v.title||'Analysis Chart')+'</div><div class="rich-chart"><svg viewBox="0 0 '+w+' '+h+'" width="100%" role="img" aria-label="'+escHtml(v.title||'chart')+'">'+rows+'</svg></div></div>');
    }
    if(v.type==='flow'){
    ...
    if(v.type==='table'){
      ...
      blocks.push('<div class="rich-block"><div class="rich-title">'+escHtml(v.title||'Analysis Table')+'</div><div class="tablewrap"><table class="rich-table"><thead><tr>'+headers.map(h=>'<th>'+escHtml(h)+'</th>').join('')+'</tr></thead><tbody>'+rows.map(row=>'<tr>'+headers.map((_,i)=>'<td>'+escHtml(Array.isArray(row)?row[i]:'')+'</td>').join('')+'</tr>').join('')+'</tbody></table></div></div>');
    }
    if(v.type==='callout')blocks.push('<div class="rich-block"><div class="rich-title">'+escHtml(v.title||'Recommendation')+'</div><div class="rich-copy">'+escHtml(v.text||'')+'</div></div>');
  });
  if(d.recommendation)blocks.push('<div class="rich-block"><div class="rich-title">Recommended Action</div><div class="rich-copy">'+escHtml(d.recommendation)+'</div></div>');
  return '<div class="rich-answer">'+blocks.join('')+'</div>';
}
...
async function askAdvisorFreeText(){
  const input=document.getElementById('question'),ans=document.getElementById('aiAnswer'),q=String(input.value||'').trim();
  if(!q)return;
  ans.value='Thinking with the current project and document context...';
  advisorHistory.push({role:'user',content:q});
  try{
    const context=projectChatContext();
    context.advisorMode=true;
    context.instruction='Answer the user naturally like a general-purpose GPT assistant, using the current project, uploaded documents, workbook intelligence and conversation context when relevant. Do not force a fixed template. Match the requested output: explanation, comparison, review, recommendation, test cases, implementation steps, draft, table, checklist, analysis, or other format as appropriate. Clearly separate known facts from assumptions and recommendations. Be concise unless the user asks for detail.';
    const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:q,history:advisorHistory.slice(-12),context})});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'Advisor request failed');
    ans.value=d.reply||d.report||'No answer returned.';
    const rich=document.getElementById('aiAnswerRich'); if(rich) rich.innerHTML=renderRichAdvisor(d);
    advisorHistory.push({role:'assistant',content:ans.value});
    document.getElementById('mode').textContent=d.mode==='ai'?'● AI Advisor Connected':'● Local Intelligence Active';
  }catch(e){
    ans.value='Advisor unavailable: '+(e.message||'Unable to analyse the request.');
  }
}
document.getElementById('askAI').onclick=askAdvisorFreeText;
document.getElementById('question').addEventListener('keydown',e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();askAdvisorFreeText()}});
const chatHistory=[];
...
async function sendAgentMessage(forced){
  ...
  try{
    ...
    const d=await r.json();msgs.lastChild.remove();
    const chatRich=document.createElement('div');chatRich.className='msg agent';chatRich.innerHTML=renderRichAdvisor(d);msgs.appendChild(chatRich);pendingAgentActions=Array.isArray(d.actions)?d.actions:[];
...
if(url.pathname==='/api/chat'&&request.method==='POST'){
  ...
  try{
    const system='You are ProjectMind AI Advisor, a senior Principal PMO, Solution Architect, Professional Test Lead and Full-Stack Reviewer embedded in a Project Intelligence dashboard. Do not give generic advice. Analyse the supplied project context and make recommendations tied to specific evidence, task names, statuses, dependencies, PICs, dates, documents and workbook intelligence when available. Explain why the issue matters, its impact, the root cause or gap, the recommended solution, concrete implementation steps, testing evidence, monitoring controls and escalation triggers. Distinguish facts from inference and never invent missing data.  Understand free-form natural language in any wording, especially Indonesian and English. Answer the actual user request directly and adapt the response format to the request instead of forcing a fixed template. You may explain, analyse, compare, troubleshoot, review documents, propose solutions, draft outputs, create test scenarios, review implementation, discuss monitoring/PMO, or handle ordinary questions. Use supplied project and document context as the source of truth for project-specific facts and never invent missing facts. Clearly distinguish observed facts, assumptions and recommendations when relevant. If the user asks for a specific format, follow it. Keep answers concise by default, but provide detail when the request needs it. Return STRICT JSON only with this schema: {"reply":"professional evidence-based answer","recommendation":"specific recommended action with rationale, owner/timing/acceptance where available","visuals":[{"type":"chart","title":"...","labels":["..."],"values":[1,2]},{"type":"flow","title":"...","nodes":[{"label":"Step 1"},{"label":"Step 2"}]},{"type":"table","title":"...","headers":["...","..."],"rows":[["...","..."]]},{"type":"callout","title":"...","text":"..."}],"actions":[{"type":"update_task","task":"exact task name","status":"","priority":"","pic":"","eta":"","action":"","dependency":"","impact":"","issue":""}]}. Include visuals whenever they materially improve understanding: use charts for quantitative status/risk/progress comparisons, flow diagrams for solution/implementation/process architecture, tables for comparisons/gaps/test coverage, and callouts for key recommendations. Do not create meaningless visuals. {"reply":"natural free-form answer to the user","actions":[{"type":"update_task","task":"exact task name","status":"","priority":"","pic":"","eta":"","action":"","dependency":"","impact":"","issue":""},{"type":"create_need_attention","task":"","stream":"","status":"At Risk","priority":"High","pic":"","eta":"","action":"","dependency":"","impact":"","issue":""}]}. Actions are optional and only propose them when the user explicitly asks to change/create/update/execute something. Never claim an action has been applied; the UI requires user confirmation.'
